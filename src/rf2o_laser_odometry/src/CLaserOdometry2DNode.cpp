...
void CLaserOdometry2DNode::publish()
{
  ...
  if (!std::isfinite(rf2o_ref.lin_speed) || !std::isfinite(rf2o_ref.ang_speed)) {
    RCLCPP_WARN(get_logger(), "RF2O computed NaN/Inf velocity! Skipping publish to protect EKF.");
    return;
  }

  const Eigen::Matrix<float, 3, 3>& cov = rf2o_ref.getIncrementCovariance();
  
  if (cov.array().isNaN().any() || cov.array().isInf().any()) {
    RCLCPP_WARN(get_logger(), "RF2O computed NaN/Inf covariance! Skipping publish to protect EKF.");
    return;
  }
  ...
  odom.header.stamp = current_time;   // the time of the last scan used!
  ...
  const double COV_SCALE = 1000.0; 
  const double COV_MIN   = 1e-3;

  odom.twist.covariance[0]  = (cov(0,0) * COV_SCALE) + COV_MIN; // vx, vx (Diagonal + Floor)
  odom.twist.covariance[1]  = cov(0,1) * COV_SCALE;             // vx, vy
  odom.twist.covariance[5]  = cov(0,2) * COV_SCALE;             // vx, w

  // Row 1 (vy)
  odom.twist.covariance[6]  = cov(1,0) * COV_SCALE;             // vy, vx
  odom.twist.covariance[7]  = (cov(1,1) * COV_SCALE) + COV_MIN; // vy, vy (Diagonal + Floor)
  odom.twist.covariance[11] = cov(1,2) * COV_SCALE;             // vy, w

  // Row 5 (w / yaw rate)
  odom.twist.covariance[30] = cov(2,0) * COV_SCALE;             // w, vx
  odom.twist.covariance[31] = cov(2,1) * COV_SCALE;             // w, vy
  odom.twist.covariance[35] = (cov(2,2) * COV_SCALE) + COV_MIN; // w, w (Diagonal + Floor)

  // Tiny variance for unknown dimensions (Z, Roll, Pitch) to prevent singularities
  odom.twist.covariance[14] = 1e-6; 
  odom.twist.covariance[21] = 1e-6; 
  odom.twist.covariance[28] = 1e-6; 

  // Copy to Pose covariance (optional, but good for some planners)
  odom.pose.covariance[0]  = odom.twist.covariance[0];
  odom.pose.covariance[7]  = odom.twist.covariance[7];
  odom.pose.covariance[35] = odom.twist.covariance[35];
  ...
  /* if (publish_tf)
  {
    ...
    odom_trans.header.stamp = current_time;    // the time of the last scan used!
