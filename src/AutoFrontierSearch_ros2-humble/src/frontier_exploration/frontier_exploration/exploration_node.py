...
class FrontierExplorationNode(Node):
    ...
    def publish_goal(self, goal):
        ...
        # FIX: The goal is already in real-world coordinates (meters). 
        # Do not multiply by resolution again!
        goal_msg.pose.position = Point(x=goal[0], y=goal[1], z=0.0)
        
        # Orientation (w=1.0 is essential, otherwise it's invalid)
        goal_msg.pose.orientation.w = 1.0 

        self.goal_publisher.publish(goal_msg)
        self.get_logger().info(f"Goal Published: x={goal_msg.pose.position.x:.2f}, y={goal_msg.pose.position.y:.2f}")
